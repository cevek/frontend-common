...
(function (w, d){
    var prot = ( (/^((https|chrome-extension):)/i).test(w.location.protocol) ? 'https' : 'http'),
        u = prot + '://share.pluso.ru', uid="1373128548227",
    ...
    //http://share.pluso.ru/img/pluso-like/square/big/08.png
    //http://share.pluso.ru/img/pluso-like/round/big/08.png
    ...
    pluso.css=[[["",".pluso"],"position:relative;z-index:1;display:inline-block;padding:0;-webkit-border-radius:22px;-moz-border-radius:22px;border-radius:22px;background:transparent;text-align:left;font-size:0;line-height:0;*display:inline;*zoom:1;"]
        ...
        ,[[832,""],"width:49px;"]
        ,[[784,""],"width:69px;"]
        ,[[32,"a.pluso-more"],"width:40px;height:40px;background:url({i}\/img\/pluso-like\/plus.png) 0 -60px no-repeat!important;position:relative;"]
        ,[[64,"a.pluso-more"],"width:20px;height:20px;background:url({i}\/img\/pluso-like\/plus.png) 0 0 no-repeat!important;position:relative;"]
        ,[[16,"a.pluso-more"],"width:30px;height:30px;background:url({i}\/img\/pluso-like\/plus.png) 0 -25px no-repeat!important;position:relative;"]
    ...
    pluso.titles = [
        ...
        /*['webmoney','Webmoney события',45,39, false],
         ['misterwong','Мистер Вонг',44,40, false],
         ['friendfeed','Friend Feed',43,41, false]*/
    ];
        ...
        for(var i=0,l=ep.services.length,q=l/2;i<l;i++)
            if (typeof pluso.services[ep.services[i]]!='undefined')
            {
                ...
                if(o.sepcounter==1){
                    ...
                    servWrapper.innerHTML = serv + '<b title = "0">0</b>';
    ...
    pluso.setCounter = function (c, url, buy, counters, advType)
    {
        ...
        var runAdvertisement = function(p) {

            var adScript = document.createElement('script');

            adScript.onload = function(e){
                if(adScript.nextSibling){
                    var advTargets = adScript.nextSibling.getElementsByTagName('a');
                    for(var i = 0; i < advTargets.length; i++)
                        if(advTargets[i].getAttribute('href') && advTargets[i].getAttribute('href').length > 0)
                            advTargets[i].onclick = function(){
                                js(u+"/process?act=ads&ev=click");
                            }
                }
            }
            ...
            var addate = new Date();
            adScript.src="http://ads.adfox.ru/214928/prepareCode?pp=g&ps=bldg&p2=" + p + "&pct=a&plp=a&pli=a&pop=a&pr=" + pr + "&pt=b&pd=" + addate.getDate() + "&pw=" + addate.getDay() + "&pv=" + addate.getHours() + "&prr=" + afReferrer;
            d.getElementsByTagName('body')[0].appendChild(adScript);
        ...
        if(!pluso.advs && advType) {
            ...
            js(u+'/adfox.sync.pixel.js', function(){
    ...
    pluso.market = function (items)
    {
        ...
        {
            var it=items[i];
            html.push('<li data-id="'+it.id+'"><a href="'+it.url+'" target="_blank"><span><img src="'+it.picture+'" border="0" alt="" height="150"></span><strong>'+it.model+'</strong><em>'+(it.price*1).toString()+' руб.<u>'+it.store.name+'</u></em><b>Купить!</b></a></li>');
        }
